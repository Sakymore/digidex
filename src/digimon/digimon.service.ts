...
export class DigimonService {
  ...
  findAll(paginationDto: PaginationDto) {
    const { limit = this.defaultLimit, offset = 5 } = paginationDto;

    return this.digimonModel.find().limit(limit).skip(offset);
  }
  ...
  async findOne(term: string) {
    ...
    if (!digimon) {
      digimon = await this.digimonModel.findOne({
    ...
    if (!digimon)
      throw new NotFoundException(
        `Digimon con el id, nombre "${term}" no encontrado`,
      );
  ...
  async remove(id: string) {
    ...
    if (deletedCount === 0) {
      throw new BadRequestException(
        `El digimon con el id "${id}" no fue encontrado `,
      );
    }
    return;
  }
  ...
  private handelExceptions(error: any) {
    if (error.code === 11000) {
      throw new BadRequestException(
        `Digimon existe en la base de datos ${JSON.stringify(error.keyValue)}`,
      );
    }
    console.log(error);
    throw new InternalServerErrorException(
      `No se puede crear Digimon - Revisa el log del servidor`,
    );
  }
}
